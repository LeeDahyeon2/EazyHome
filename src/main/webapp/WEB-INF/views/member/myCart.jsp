...
<c:set var = "num" scope = "session" value = "${cartList.size()}"/>
<c:choose>
<c:when test = "${num==0}">
...
<span class="gLeft">[기본배송]</span> 상품구매금액 ${total_price}원  + 배송비 <span id="normal_normal_ship_fee">0원 (무료)</span> = 합계 : <strong class="txtEm gIndent10"><span id="normal_normal_ship_fee_sum" class="txt18">${total_price}</span>원</strong> 
</c:when>
<c:otherwise>
<span class="gLeft">[기본배송]</span> 상품구매금액 ${total_price}원  + 배송비 <span id="normal_normal_ship_fee">2500원</span> = 합계 : <strong class="txtEm gIndent10"><span id="normal_normal_ship_fee_sum" class="txt18">${total_price+2500}</span>원</strong> 
</c:otherwise>
</c:choose>
</td>
                </tr></tfoot> 
			...
			<c:forEach var="cart" items="${cartList}">
                <tr class="xans-record-">
                    ...
                    <c:choose>
                    <c:when test="${cart.quantity * cart.price>=50000}">
                    ...
                    <td class="button">
                        <a href="../item/order?basketType=a&itemCode=${cart.itemCode}" class="btnNormal" ><i class="icoDelete"></i>주문</a>
                    </td>
                    <td class="button">	
                        <a href="./cartDel?itemCode=${cart.itemCode}" class="btnNormal" ><i class="icoDelete"></i>삭제</a>
                    </td>
                </tr>
                </c:forEach>
</tbody> 
...
<div class="xans-element- xans-order xans-order-basketpriceinfoguide  "><p class="info ">5만원 이상 구매시 무료배송입니다.</p>
</div>
<!-- 총 주문금액 : 국내배송상품 -->
...
<c:choose>
<c:when test="${total_price>=50000}"><c:set var="total_del" scope="session" value="0"/></c:when>
<c:otherwise><c:set var="total_del" scope="session" value="2500"/></c:otherwise>
</c:choose>
<thead><tr>
<th scope="col" style="text-align:center"><strong>총 상품금액</strong></th>
                    <th scope="col" style="text-align:center"><strong>총 배송비</strong></th>
                    <th scope="col" style="text-align:center"><strong>결제예정금액</strong></th>
                </tr></thead>
<tbody class="center"><tr>
<td><div class="box txt16">
<strong><span class="txt23">${total_price}</span>원</strong> 
</div></td>
                    <td>
                        <div class="box shipping txt16">
                            <strong class="txt23">+ </strong><strong><span id="total_delv_price_front" class="txt23">${total_del}</span>원</strong> 
                        </div>
                    </td>
                    <td><div class="box txtEm txt16">
<strong class="txt23">= </strong><strong><span id="total_order_price_front" class="txt23">${total_price+total_del}</span>원</strong>
</div></td>
                </tr></tbody>
</table>
...
<!-- 총 주문금액 : 국내배송상품 -->
<!-- 총 주문금액 : 해외배송상품 -->
<div class="xans-element- xans-order xans-order-totalorder ec-base-button justify"><a href="../item/order" class="btnSubmitFix sizeM  ">전체상품주문</a>
		<span class="gRight">
            <a href="../item/itemList" class="btnNormalFix sizeM">쇼핑계속하기</a>
        </span>
<!-- 네이버 체크아웃 구매 버튼  -->
